		...
		try {
			...
			if (null == userDaoImpl.verifyUser(request.getParameter("login"), request.getParameter("password"))
				...
				if (null == request.getSession().getAttribute("user_id")) {
					request.setAttribute("login", u.getAuthenticate().getLogin());
					request.setAttribute("password", u.getAuthenticate().getPassword());
					request.getRequestDispatcher("LibraryJDBCServlets/front-controller/?Command=Login").forward(request,
							response);
				}
				forward("userCabinet");
			} else {
